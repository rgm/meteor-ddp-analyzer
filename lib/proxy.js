...
module.exports = function(meteorPort, proxyPort, throttle) {
  ...
  var server = http.createServer(handleBrowserRequests(middleware, meteorPort, throttle));
  sockjsServer.on('connection', onBrowserConnection(meteorPort, throttle));
...
function onBrowserConnection(meteorPort, throttle) {
  return function(conn) {
    ...
    conn.on('data', function(message) {
      console.log('delaying data by ',throttle);
      setTimeout(function() {
...
function handleBrowserRequests(sockjsMiddleware, meteorPort, throttle) {
